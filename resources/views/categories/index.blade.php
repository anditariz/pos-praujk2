...
<div class="mb-6 flex justify-between items-center animate-fadeIn">
    ...
    <button id="open-modal" class="px-4 py-2 bg-primary-blue text-white rounded-md hover:bg-opacity-90 transition-all duration-200 transform hover:scale-105 shadow-md eg-modal-toogle" data-id="createCategoriesModal">
...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden mb-6 animate-fadeIn" style="animation-delay: 0.5s;">
    {{-- @if ($privilage[0]['create']) --}}
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
            <h2 class="text-lg font-semibold">Add Categories</h2>
                ...
                @foreach ($datas as $data)
                    <tr class="table-row">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2 inline-flex text-xs leading-5 rounded-full">{{$data->category_name}}</span>
                        </td>
                        @if ($privilage[0]['update'] || $privilage[0]['delete'])
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    @if ($privilage[0]['delete'])
                                        <button class="text-primary-blue hover:text-primary-blue/80 mr-3 transition-colors"><i class="fas fa-trash eg-modal-toogle" data-id="deleteCategoriesModal{{$data->id}}"></i></button>
                                    @endif
                                    @if ($privilage[0]['update'])
                                        <button class="text-primary-green hover:text-primary-green/80 transition-colors"><i class="fas fa-edit eg-modal-toogle" data-id="editCategoriesModal{{$data->id}}"></i></button>
                                    @endif
                            </td>
                        @endif
                    </tr>
