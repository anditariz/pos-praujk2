            ...
            product.name
                ...
                <div class="p-3">
                    ...
                    <div class="flex justify-between items-center mt-3">
                        <span class="font-bold text-indigo-600">Rp. ${product.price.toFixed(
                            2
                        )}</span>
    ...
    cart.forEach((item) => {
        ...
        <div class="flex items-center space-x-3">
            ...
            <div>
                ...
                <p class="text-xs text-gray-500">Rp.${item.price.toFixed(2)}</p>
            </div>
        </div>
        <div class="flex items-center space-x-3">
...
function showReceipt(order) {
    ...
    order.items.forEach((item) => {
        ...
        <div class="col-span-2 text-right">Rp.${item.price.toFixed(2)}</div>
        <div class="col-span-2 text-right">Rp.${itemTotal.toFixed(2)}</div>
    `;

        receiptItems.appendChild(itemRow);
    });

    receiptSubtotal.textContent = `Rp.${order.subtotal.toFixed(2)}`;
    receiptTax.textContent = `Rp.${order.tax.toFixed(2)}`;
    receiptTotal.textContent = `Rp.${order.total.toFixed(2)}`;
